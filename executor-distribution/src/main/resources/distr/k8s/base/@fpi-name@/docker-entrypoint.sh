...
#настройка SSL
CLIENT_CERTS_PATH="/vault/secrets/ssl/crt/tls.crt"
CLIENT_PRIVATE_KEY_PATH="/vault/secrets/ssl/crt/tls.key"
SERVER_CERTS_PATH="/vault/secrets/ssl/ca/root.crt"
...
echo -n $SSL_PASSWORD > $KEY_STORE_PASSWORD_PATH
echo "Создание файла с паролем для keystore: финиш"

echo "Создание файла с паролем для trustStore: старт"
echo -n $SSL_PASSWORD > $TRUST_STORE_PASSWORD_PATH
echo "Создание файла с паролем для trustStore: финиш"

echo "chmod - на чтение секретов: старт"
chmod go-r $KEY_STORE_PATH
chmod go-r $KEY_STORE_PASSWORD_PATH
chmod go-r $TRUST_STORE_PATH
chmod go-r $TRUST_STORE_PASSWORD_PATH
echo "chmod - на чтение секретов: финиш"
